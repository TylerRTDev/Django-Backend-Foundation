...
### 1. Custom User Model
Django’s default `User` model uses a username for authentication, which can be restrictive. In this foundation, we replace it with a **custom User model** that uses an email address for login. This approach is more modern and aligns with most real-world web apps.
...
- Split `settings.py` into modular files: `base.py`, `dev.py`, `prod.py`.
- Migrate from SQLite to PostgreSQL.
- Add Whitenoise for production static file serving.
- Integrate Cloudflare for global static/media caching.
- Extend the API layer with `PATCH /api/me/{id}/` for profile updates.
- Add authentication endpoints (login, logout, password reset, registration) via DRF.
...
Understanding these principles now will save massive time and headaches when your projects grow from small experiments into full-scale applications.
